...
define('WPWEBMAPS_PLUGIN_VERSION', '0.2.2');
...
if ( ! function_exists( 'wpwebmaps_enqueue_and_register' ) ) {
	function wpwebmaps_enqueue_and_register(){
		...
		$jquery_url = WPWEBMAPS_PLUGIN_URL.'vendor/jquery-3.5.1.min.js';
		$js_url = WPWEBMAPS_PLUGIN_URL.'vendor/leaflet-1.6.0.js';
		$css_url = WPWEBMAPS_PLUGIN_URL.'vendor/leaflet-1.6.0.css';
		$apiGoogle='https://maps.googleapis.com/maps/api/js?key='.html_entity_decode($wpwebmaps_options['google_api_0']).'';
		$jsGMutant_url = WPWEBMAPS_PLUGIN_URL.'vendor/Leaflet.GoogleMutant-0.6.4.js';

		if ( ! wp_script_is( 'jquery', 'enqueued' )) {
			wp_register_script('jquery', $js_url, Array(), null, true);
			//Enqueue
...
if ( ! function_exists( 'wpwebmaps_shortcode' ) ) {
	function wpwebmaps_shortcode($atts) { 
		...
		wpwebmaps_enqueue_and_register();
		// 3 wp_loaded >> After WordPress is fully loaded
		wpwebmaps_inline_start();
...
*/
add_filter('single_template', 'wpwebmaps_cat_template');
 
/**
* Single template function which will choose our template
*/
function wpwebmaps_cat_template($single) {
	...
	*/
	foreach((array)get_the_category() as $cat) :
		...
		$s=plugin_dir_path( __FILE__ ) . '/templates';
		if(file_exists($s . '/tmplt_' . $cat->slug . '.php')){
			//echo '1)'.$s . '/tmplt_' . $cat->slug . '.php';
			//exit;
			return $s . '/tmplt_' . $cat->slug . '.php';
		}
